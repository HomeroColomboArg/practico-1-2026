...
export function obtenerNombresCompletos(alumnos: Alumno[]): string[] {
    return alumnos.map(a => `${a.nombre} ${a.apellido}`);
}
...
export function obtenerMayoresDeEdad(alumnos: Alumno[]): Alumno[] {
    return alumnos.filter(a => a.edad >= 18);
}
...
export function calcularPromedio(alumnos: Alumno[]): number {
    ...
    const sNotas = alumnos.reduce((acc, alumno) => {
        return acc + alumno.nota
    }, 0);

    return sNotas/alumnos.length ;
}
...
export function obtenerMejorAlumno(alumnos: Alumno[]): Alumno | undefined {
    ...
    const mejorAlumno = alumnos.reduce((acc, alumno) => {
        if (alumno.nota > acc.nota) {
            return alumno;
        }
        return acc;
    });
...
export function buscarPorLegajo(
    ...
    const alumnoBuscado = alumnos.find(a => a.legajo === legajo);
...
export function buscarPorNombre(
    ...
    // TODO
    const buscado = alumnos.find(a => a.nombre === nombre);
    return buscado;
    //throw new Error("Implementar");
}
...
export function existeDesaprobado(alumnos: Alumno[]): boolean {
    // TODO
    const desaprobados = alumnos.some(a => a.nota < 6);
    return desaprobados;
}
...
export function todosAprobaron(alumnos: Alumno[]): boolean {
    // TODO
    const aprobados = alumnos.every(a => a.nota >= 6);
    return aprobados;
}
...
export function cantidadAprobados(alumnos: Alumno[]): number {
    return alumnos.filter(a => a.nota >= 6).length;
}
...
export function sumarEdades(alumnos: Alumno[]): number {
    // TODO
    const edadTotal = alumnos.reduce((acc, alumno) => acc + alumno.edad,0);
...
export function obtenerAlumnosDeCiudad(
    ...
    // TODO
    const alumnoCiudad = alumnos.filter(a => a.ciudad === ciudad);
    return alumnoCiudad;
}
...
export function transformar<T, R>(
    ...
    return elementos.map((elemento: T): R => callback(elemento));
}
...
export function filtrar<T>(
    ...
    return elementos.filter((elemento: T): boolean => callback(elemento) === true);
}
...
export function buscar<T>(
    ...
    return elementos.find((elemento: T): boolean => callback(elemento))
}
...
export function calcularTotal(
    ...
    return alumnos.reduce((acc, alumno): number => acc += callback(alumno), 0)
}
...
export function agruparPorCiudad(
    ...
    const ciudades = alumnos.reduce((acc, alumno: Alumno) => acc.add(alumno.ciudad), new Set<string>([]));
    const objeto: Record<string, Alumno[]> = {};
    ciudades.forEach((ciudad) => objeto[ciudad] = alumnos.filter((alumno) => alumno.ciudad === ciudad))
    return objeto;
}
